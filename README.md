...
export default function Dashboard({ user, rows, setRows, onLogout }) {
  ...
  async function processFile(file) {
    ...
    try {
      ...
      setTimeout(()=>{
        ...
        setPage('charts')
        setLoading(false)
        notify(`Imported ${result.rows.length.toLocaleString()} rows`, C.green)
      }, 100)
    } catch(err) {
  ...
  // ── IMPORT SCREEN (shown right after login) ────────────
  if (!rows.length) {
    return (
      <div style={{
        ...
        {/* Import card centered on screen */}
        <div style={{
          flex:1, display:'flex',
          ...
          <div style={{ width:'100%', maxWidth:520 }}>
            ...
            <div style={{
              ...
              >
                {loading?'⏳ Importing…':'⬆ Browse & Import File'}
              </button>
  ...
  // ── MAIN DASHBOARD (shown after import) ───────────────
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding:'12px 10px 0' }}>
          ...
          >
            {loading?'⏳ Importing…':'⬆ Import New File'}
          </button>
          {error && (
            <div style={{
        ...
        {page==='charts' && (
          <div style={{ padding:'24px' }}>
            ...
            <div style={{
              ...
              <KPI label="Total"   value={m?.total.toLocaleString()||0} sub="records"           accent={C.sky}  />
              <KPI label="CSAT"    value={`${m?.csatPct||0}%`}         sub={`${m?.csatN||0} satisfied`}    accent={C.green}/>
              <KPI label="DSAT"    value={`${m?.dsatPct||0}%`}         sub={`${m?.dsatN||0} dissatisfied`} accent={C.red}  />
              <KPI label="Neutral" value={`${m?.neuPct||0}%`}          sub={`${m?.neuN||0} neutral`}       accent={C.amber}/>
            </div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ fontSize:10, color:C.sub, marginBottom:14 }}>
                  Pos: {m?.posN||0} · Neg: {m?.negN||0} · Neu: {m?.neuN||0}
                </div>
              ...
              <div style={{
                ...
                <div style={{ fontSize:10, color:C.sub, marginBottom:14 }}>
                  CSAT: {m?.csatPct||0}% · DSAT: {m?.dsatPct||0}%
                </div>
                <ResponsiveContainer width="100%" height={210}>
        ...
        {page==='data' && (
          <div style={{ padding:'24px' }}>
            <div style={{ marginBottom:20 }}>
              ...
              <p style={{ fontSize:11, color:C.sub, margin:'4px 0 0' }}>
                Sort · Filter · Export filtered/sorted data · 200 rows/page
              </p>
            </div>
